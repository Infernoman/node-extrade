...
extrade.prototype.connect = function (endpoint,args,method,callback,aux) {
	...
	if (method == 'POST') {
		...
		var api_key = (!aux) ? this.config.apiKey : this.config.apiKey1;
		var api_secret = (!aux) ? this.config.apiSecret : this.config.apiSecret1;
	...
	var options = {
		hostname: this.url,
		port: 443,
		path: '/api/' + endpoint + ((method == 'POST') ? '' : '?'+args_string2),
	...
	var req = https.request(options, function(result) {
		...
		result.on('end', function() {
			if (!data || data.search('<') >= 0) {
				callback('Couldn\'t connect to 1EX exchange.');
				return false;
			}
			...
			try {
				...
				callback('Invalid JSON received from 1EX exchange.');
				return false;
			}
